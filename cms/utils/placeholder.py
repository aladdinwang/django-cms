...
def get_placeholder_conf(setting, placeholder, template=None, default=None, group=None):
    """
    ...
    is checked.
    @wej added
    Keyword Prefix "Group GroupName"
    """

    if placeholder:
        keys = []
        if template and ("plugins" == setting or "limits" == setting):
            # although ugly
            from cms.utils.plugins import get_placeholders
            placeholder_nodes = get_placeholders(template, take_node=True)
            for placeholder_node in placeholder_nodes:
                if placeholder_node.get_name() == placeholder:
                    group = placeholder_node.get_group()
                    keys.append("%s %s" % ('group', group))
