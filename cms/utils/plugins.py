...
def _extend_nodelist(extend_node, take_node=False):
    """
    ...
    for block in blocks.values():
        placeholders += _scan_placeholders(block.nodelist, block, blocks.keys(), take_node = take_node)

    # Scan topmost template for placeholder outside of blocks
    parent_template = _find_topmost_template(extend_node)
    placeholders += _scan_placeholders(parent_template.nodelist, None, blocks.keys(), take_node = take_node)
    return placeholders

def _scan_placeholders(nodelist, current_block=None, ignore_blocks=None, take_node=False):
    placeholders = []
    ...
    for node in nodelist:
        ...
        if isinstance(node, Placeholder):
            placeholder = (take_node and [node] or [node.get_name()])[0]
        ...
        # scan the child template
            ...
            if node.template:
                placeholders += _scan_placeholders(node.template.nodelist, current_block, take_node = take_node)
        # handle {% extends ... %} tags
        elif isinstance(node, ExtendsNode):
            placeholders += _extend_nodelist(node, take_node=take_node)
        # in block nodes we have to scan for super blocks
            ...
            if node.filter_expression.token == 'block.super':
                ...
                placeholders += _scan_placeholders(current_block.super.nodelist, current_block.super, take_node = take_node)
                ...
                if hasattr(node, nodelist_name):
                    ...
                    if isinstance(subnodelist, NodeList):
                        if isinstance(node, BlockNode):
                            current_block = node
                        placeholders += _scan_placeholders(subnodelist, current_block, ignore_blocks, take_node = take_node)
        # else just scan the node for nodelist instance attributes
            ...
            for attr in dir(node):
                ...
                if isinstance(obj, NodeList):
                    if isinstance(node, BlockNode):
                        current_block = node
                    placeholders += _scan_placeholders(obj, current_block, ignore_blocks, take_node = take_node)
    return placeholders
...
def get_placeholders(template, take_node=False):
    ...
    clean_placeholders = []
    #from @wej if we get placeholder instance, what will happen
