...
from cms.forms.widgets import PluginEditor, ContentTemplateEditor
...
def contribute_fieldsets(cls):
    ...
    export_fields = ['webapp', 'exported', 'content_template_path']
    ...
    setattr(cls, 'export_fields', export_fields)
...
class PageAdmin(ModelAdmin):
    ...
    def get_urls(self):
        ...
        url_patterns = patterns('',
                                ...
                                pat(r'^(?P<object_id>\d+)/export_template/$', self.export_template), # @wej added
        )
    ...
    def save_model(self, request, obj, form, change):
        ...
        if 'recover' in request.path or 'history' in request.path:
            ...
            for placeholder_node in placeholder_nodes:
                group = placeholder_node.get_group()
                if group and (group == 'image' or False):
                    placeholder, dummy = obj.placeholders.get_or_create(slot=placeholder_node.get_name())
                    if len(placeholder.get_plugins_list()):
                        continue
                    plugin_type = 'PicturePlugin'
                    plugin_tips = placeholder_node.get_tips()
                    width = placeholder_node.get_width()
                    height = placeholder_node.get_height()
                    if not plugin_tips:
                        plugin_tips = _('Missing default tips')
                    from cms.plugins import picture
                    # generate image
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        fieldsets.
        """
        # from @wej here we need to prepopulated the image tooltip for the placeholder
        # of specified group, such as 'group'
        if obj: # edit
            ...
            # @wej here we add our section named 'content_template'
            name = _('Content Template Design')
            given_fieldsets += [(title(name), {'fields': ['content_template'], 'classes': ['module', 'aligned', 'collapse']})]
            advanced = given_fieldsets.pop(4)
    ...
    def get_fieldset_placeholders(self, template, take_node=False):
        return plugins.get_placeholders(template, take_node=take_node)
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        if obj:
            ...
            placeholder_book = {}
            for (placeholder_name, placeholder_node) in [(placeholder_node.get_name(), placeholder_node) for placeholder_node in placeholders]:
                plugin_list = []
                show_copy = False
                copy_languages = {}
                if versioned:
                    ...
                    # from @wej important! obj.placeholders.get_or_create, parameter is placeholder_name
                    # import pdb
                    # pdb.set_trace()
                    placeholder, created = obj.placeholders.get_or_create(slot=placeholder_name)
                    print "page_id: %s, placeholder_id: %s" % (obj.pk, placeholder.pk)
                    if created:
                        plugin_group = placeholder_node.get_group()
                        if plugin_group and plugin_group == 'image':
                            plugin_type = 'PicturePlugin'
                            plugin_tips = placeholder_node.get_tips()
                            width = placeholder_node.get_width()
                            height = placeholder_node.get_height()
                            ...
                            default_img_path = get_cms_setting('PAGE_MEDIA_PATH') + 'default/' + 'photo1.jpg'
                            # image = None
                            asy_template = get_template(settings.DEFAULT_IMAGE_ASY_FILE)
                            asy_content = asy_template.render(Context({'tips': plugin_tips, }))
                ...
                if copy_languages and len(get_language_list()) > 1:
                    show_copy = True
                # from @wej plz check PluginEditor
                ...
                # @wej modify 'content_template' base field
                content_template_url = reverse('admin:cms_page_preview_page',
                                               args=(obj.pk,),
                                               current_app=request.session.get('cms_admin_site', None))
                # @wej only hardcode testing
                content_template_url += "?preview=1&design=1&language=" + language
                content_template_widget = ContentTemplateEditor(
                    attrs={
                        'copy_languages': copy_languages.items(),
                        'language': language,
                ...
                form.base_fields['content_template'] = CharField(widget=content_template_widget, required=False, label=_('Content Template Short Description'))
    ...
    def change_view(self, request, object_id, extra_context=None):
        ...
        try:
            ...
            # from @wej get_template_from_request just return the name(string) of
            # of the template
            selected_template = get_template_from_request(request, obj)

            #activate(user_lang_set)
            export_template_url = reverse('admin:cms_page_export_template',
                                          args=(obj.pk,),
                                          current_app=request.session.get('cms_admin_site', None))
            # important!
            export_template_url += "?preview=1&design=1"
            context = {
        ...
        # from @wej @lookat options.py:1064
    ...
    def export_template(self, request, object_id):
        page = get_object_or_404(Page, pk=object_id)
        
        # @wej authority checking
        content_template_url = reverse('admin:cms_page_preview_page',
                                       args=(page.pk,),
                                       current_app=request.session.get('cms_admin_site', None))
        language = get_language_from_request(request)
        attrs = "?preview=1&draft=1&language=" + language
        content_template_url += attrs
        ...
        content_template_path = page.content_template_path
        project_base = settings.PROJECT_PATH + "/"
        
        path =  project_base + content_template_path
        path = "/home/aladdin/jiapin/demo/tests/templates/master/" \
            + content_template_path
        wpath = os.path.dirname(path)
        if not os.path.exists(wpath):
            os.makedirs(wpath)
        fp = open(path, 'w')
        fp.write(render_to_string(template_name, context))
        fp.close()
    ...
    def preview_page(self, request, object_id):
        ...
        if request.REQUEST.get('design', None):
            attrs += "&design=1"
    ...
    def edit_plugin(self, request, plugin_id):
        ...
        if not 'history' in request.path and not 'recover' in request.path:
            ...
            # @wej @debug
            #print type(instance), type(plugin_admin), type(cms_plugin)
            if page and not page.has_change_permission(request):
                return HttpResponseForbidden(_("You do not have permission to change this page"))
        else:
            # history view with reversion
            # from @wej here, we should check our modification will influence something in it.
            from reversion.models import Version
